...
def prog2events (prog : LitmusAST.Program) (start_id : Nat) : List Primitives.Event × Nat :=
  let thread : Primitives.Thread := Primitives.Thread.mk prog.thread
  let ins2event := ins2event prog.thread thread

  let rec process_instructions (instructions : List LitmusAST.Instruction) (curr_id : Nat) : List Primitives.Event × Nat :=
...
def zip_instructions (insts : List Primitives.Event) : Set (Primitives.Event × Primitives.Event) :=
...
def get_po (litmus : LitmusAST.Litmus) : Primitives.Relation :=
  let start : Nat := 0

  let rec process_programs (program : List LitmusAST.Program) (curr_id : Nat) : List (List Primitives.Event) × Nat :=
    match program with
    | [] => ([], curr_id)
    | prog :: rest =>
...
def get_fr (litmus : LitmusAST.Litmus) : Primitives.Relation :=
  sorry
